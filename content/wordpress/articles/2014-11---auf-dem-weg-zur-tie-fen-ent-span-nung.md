...
<RemoteImage alt={`Auf dem Weg zur Tiefenentspannung`} size="medium"
...
<RemoteImage alt={`Eine kurze Nackenmassage später`} size="small"
...
<RemoteImage alt={`keep calm and relax your shoulders`} size="small"
...
<RemoteImage alt={`Tiefenentspannung`} size="medium"
...
<RemoteImage alt={`Entspannung`} size="small" title={`Entspannung`}
...
<RemoteImage alt={`Haben sie Kopfschmerzen?`} size="medium" title={``}
...
<RemoteImage alt={`Ab jetzt gibt es im Hause Reis jeden Tag einen Kräutertrunk`}
size="medium"

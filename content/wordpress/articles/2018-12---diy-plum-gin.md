...
<RemoteImage alt={`Likoer`} size="medium" title={`Die Grundlage für den Likör`}
...
<RemoteImage alt={`Likör`} size="medium" title={`Der Ansatz`}
...
<RemoteImage alt={`Likör`} size="medium" title={`Das große Finale`}
